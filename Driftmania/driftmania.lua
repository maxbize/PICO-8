  ...
  for i = 1, #checkpoints do
    ...
    for j = 1, cp.l do
      ...
      if last_sprite_x ~= sprite_x or last_sprite_y ~= sprite_y then
        local sprite_index = map_decl_tiles[sprite_x][sprite_y]
...
local decl_chunk_map = {6,10,3,12,13,6,13,6,13,6,6,6,6,13,13,6,6,6,6,13,6,6,13,6,6,6,6,13,0,11,9,3,12,12,12,12,10,10,10,10,10,10,9,9}
function draw_minimap()
  local camera_x = peek2(0x5f28)
  local camera_y = peek2(0x5f2a)
  local offset = 128 - map_settings.size
  ...
  for i = 0, map_settings.size - 1 do
    for j = 0, map_settings.size - 1 do
      ...
      local decl_chunk_index = map_decl_chunks[i][j]
      if decl_chunk_index > 0 and road_chunk_map[road_chunk_idx] ~= 13 then
        pset(offset + camera_x + i, offset + camera_y + j, decl_chunk_map[decl_chunk_index])
      end
    end
  end
